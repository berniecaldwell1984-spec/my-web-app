...
const Icons = {
  ...
  Truck: (props) => <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}><rect x="1" y="3" width="15" height="13" rx="2" ry="2"/><polygon points="16 8 20 8 23 11 23 16 16 16 16 8"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg>
};
...
// Input Field: Standardized White Box, Black Text
  ...
  return (
    <div className={`mb-4 ${className}`}>
      ...
      <input 
        ...
        className="w-full bg-white border border-gray-300 rounded-lg p-3 text-base text-black font-bold focus:border-green-600 focus:ring-2 focus:ring-green-600 focus:outline-none placeholder-gray-400 transition-colors shadow-sm"
        {...props}
      />
    </div>
  );
    ...
    <div className="mb-4">
      ...
      <div className="relative">
        <select 
          value={value} 
          onChange={e => onChange && onChange(e.target.value)} 
          className="w-full bg-white border border-gray-300 rounded-lg p-3 text-base text-black font-bold focus:border-green-600 focus:ring-2 focus:ring-green-600 focus:outline-none appearance-none shadow-sm"
        >
          {children}
        </select>
        <div className="absolute right-3 top-3 text-black pointer-events-none"><Icon size={16} className="rotate-90" /></div>
      </div>
    </div>
  );
};
...
const INITIAL_INSPECTIONS = [
  { id: 1, title: "Foundation Concrete Pour", location: "Building A - Foundation", date: "2025-11-29", result: "Pass", inspector: "Bernie Caldwell", category: "Concrete", notes: "Concrete met all specifications.", photos: [] }
];

const INITIAL_TESTS = [
  { id: 1, title: "Concrete Compressive Strength", category: "Concrete", procedure: "TR 230", location: "Building A", date: "2025-11-29", result: "Pass", value: "3250", unit: "PSI", spec: "Min 3000 PSI" }
];
    ...
    <div className="fixed inset-0 bg-black/90 backdrop-blur-sm z-[100] flex items-center justify-center p-4 print:hidden">
      <div className="bg-zinc-900 w-full max-w-md rounded-xl border border-zinc-800 shadow-2xl flex flex-col max-h-[90vh]">
        <div className="p-4 border-b border-zinc-800 flex justify-between items-center">
          ...
          <button onClick={onClose}><Icons.X className="text-zinc-500 hover:text-white" /></button>
        </div>
        <div className="p-6 overflow-y-auto space-y-4">
          ...
          <div>
            <label className="text-[10px] font-bold text-zinc-500 uppercase mb-1 block tracking-wide">NOTES</label>
            <textarea 
          ...
          <div>
             <label className="text-[10px] font-bold text-zinc-500 uppercase mb-1 block tracking-wide">PHOTO EVIDENCE</label>
        ...
        <div className="p-4 border-t border-zinc-800 flex gap-3">
          <button onClick={onClose} className="flex-1 py-3 text-xs font-bold uppercase text-zinc-500 hover:text-white hover:bg-zinc-800 rounded transition-colors">Cancel</button>
          <button onClick={() => onSave(form)} className="flex-1 py-3 bg-green-600 text-white text-xs font-bold uppercase rounded hover:bg-green-500 transition-colors">Save Inspection</button>
        </div>
      </div>
    </div>
  );
...
function TestModal({ onClose, onSave }) {
  const [form, setForm] = useState({
    title: '', category: '', procedure: '', location: '', 
    date: new Date().toISOString().split('T')[0], 
    value: '', unit: '', result: 'Pass'
  });
  ...
  return (
    <div className="fixed inset-0 bg-black/90 backdrop-blur-sm z-[100] flex items-center justify-center p-4 print:hidden">
      <div className="bg-zinc-900 w-full max-w-md rounded-xl border border-zinc-800 shadow-2xl flex flex-col max-h-[90vh]">
        <div className="p-4 border-b border-zinc-800 flex justify-between items-center">
          <h3 className="text-lg font-bold text-white uppercase tracking-wider">New Material Test</h3>
          <button onClick={onClose}><Icons.X className="text-zinc-500 hover:text-white" /></button>
        </div>
        <div className="p-6 overflow-y-auto space-y-4">
          ...
          <SelectField label="TEST PROCEDURE" value={form.procedure} onChange={v => setForm({...form, procedure: v})}>
          ...
          <div className="grid grid-cols-2 gap-4">
             <InputField label="VALUE" value={form.value} onChange={v => setForm({...form, value: v})} />
        ...
        <div className="p-4 border-t border-zinc-800 flex gap-3">
          <button onClick={onClose} className="flex-1 py-3 text-xs font-bold uppercase text-zinc-500 hover:text-white hover:bg-zinc-800 rounded transition-colors">Cancel</button>
          <button onClick={() => onSave(form)} className="flex-1 py-3 bg-green-600 text-white text-xs font-bold uppercase rounded hover:bg-green-500 transition-colors">Save Test</button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="space-y-8 animate-in fade-in">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <QuickActionBtn iconName="Calculator" label="Soil Cement Calc" onClick={() => setActiveTab('soilcalc')} />
          <QuickActionBtn iconName="Truck" label="Asphalt Yield Calc" onClick={() => setActiveTab('asphaltcalc')} />
...
function AIAssistantView() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'model', text: "Error: Check your API Key or Internet Connection." }]);
    } finally {
      setIsLoading(false);
    }
  };
...
function SoilCalcView({ segments, addSegment, removeSegment, current, setCurrent }) {
  ...
  const handlePhotoSelect = (e) => {
    if (e.target.files && e.target.files[0]) {
        setCurrent({ ...current, photo: URL.createObjectURL(e.target.files[0]) });
    }
  };
  ...
  const calculate = (seg) => {
    ...
    const d = parseFloat(seg.depth) || 0;
    const rho = parseFloat(seg.density) || 0;
    const pct = parseFloat(seg.percent) || 0;
    const areaSqYd = (l * w) / 9;
    const volumeCF = (l * w * (d / 12));
    const totalLbs = volumeCF * rho * (pct / 100);
    const tons = totalLbs / 2000;
    const spreadRate = areaSqYd > 0 ? (totalLbs / areaSqYd) : 0;
    return { areaSqYd, volumeCF, totalLbs, tons, spreadRate };
  };
  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-bottom-4">
       ...
       <style>{`
         @media print {
           body, html, #root { background-color: #000000 !important; -webkit-print-color-adjust: exact; print-color-adjust: exact; color: white !important; }
           .print\\:hidden { display: none !important; }
           .print\\:block { display: block !important; }
           .print\\:bg-black { background-color: #000000 !important; }
           .print\\:text-white { color: #ffffff !important; }
           .print\\:border-green-600 { border-color: #16a34a !important; }
           .print\\:w-full { width: 100% !important; }
           .print\\:absolute { position: absolute !important; top: 0; left: 0; }
       ...
       <div className="print:hidden">
         <div className="flex justify-between items-center border-b border-zinc-800 pb-4 mb-4">
          <h2 className="text-3xl font-black text-white uppercase tracking-tighter">Soil Cement & Lime</h2>
          <div className="text-xs bg-zinc-900 border border-zinc-800 px-3 py-1 rounded text-zinc-500 font-mono">Factors: Cement 94lb | Lime 35lb</div>
        </div>
        <div className="grid grid-cols-2 gap-4">
            <InputField label="JOB NAME / NUMBER" placeholder="e.g. I-49 Rehabilitation" />
      ...
      <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-xl shadow-lg print:hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4 p-4 bg-black rounded-lg border border-zinc-800">
            <InputField label="START STATION" placeholder="100+00" value={current.startStation || ''} onChange={(v) => setCurrent({...current, startStation: v})} />
            <InputField label="END STATION" placeholder="105+00" value={current.endStation || ''} onChange={(v) => setCurrent({...current, endStation: v})} />
            <InputField label="CALC LENGTH (FT)" value={current.length} onChange={(v) => setCurrent({...current, length: v})} readOnly className="opacity-75" />
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <InputField label="WIDTH (FT)" value={current.width} onChange={(v) => setCurrent({...current, width: v})} />
            <InputField label="DEPTH (IN)" value={current.depth} onChange={(v) => setCurrent({...current, depth: v})} />
        </div>
        ...
        <div className="mb-4">
            ...
            <button 
                onClick={() => fileInputRef.current.click()}
                className={`w-full py-3 rounded-lg border-2 border-dashed flex items-center justify-center gap-2 transition-colors ${current.photo ? 'border-green-600 text-green-500 bg-green-900/20' : 'border-zinc-700 text-zinc-500 hover:border-zinc-500'}`}
            >
                <Icons.Camera size={20} />
                <span className="font-bold text-xs uppercase">{current.photo ? "Photo Attached" : "Attach Photo"}</span>
            </button>
        </div>
      ...
      {segments.length > 0 && (
         <div className="bg-black border-2 border-green-600 rounded-xl overflow-hidden shadow-2xl print:border-green-600 print:shadow-none print:w-full print:absolute print:top-0 print:left-0 print:z-50 print:bg-black">
            <div className="p-6 border-b-2 border-green-600 bg-black print:bg-black flex justify-between items-center">
                <div className="flex items-center gap-4">
                    <img src="/Gilchrist_Logo.png" alt="GILCHRIST" className="h-12 object-contain" onError={(e) => e.target.style.display='none'}/>
                    <div>
                        <h3 className="font-black text-white print:text-white text-xl uppercase tracking-tighter leading-none">Soil & Lime Report</h3>
                        <p className="text-green-500 text-xs font-bold uppercase tracking-widest mt-1">Material Stabilization</p>
                    </div>
                </div>
                <div className="print:hidden">
                    <button onClick={() => window.print()} className="flex items-center gap-2 text-[10px] font-bold uppercase text-black bg-white px-4 py-2 rounded hover:bg-gray-200 transition-colors"><Icons.Printer size={14}/> Print Report</button>
                </div>
            </div>
            {segments.map(seg => {
                const res = calculate(seg);
                return (
                    <div key={seg.id} className="p-6 border-b border-zinc-800 grid grid-cols-1 md:grid-cols-12 gap-6 items-center bg-black print:bg-black hover:bg-zinc-900/30 transition-colors">
                        <div className="md:col-span-4">
                            <span className="block font-black text-2xl text-white print:text-white uppercase mb-1">{seg.material} {seg.percent}%</span>
                            <span className="inline-block text-[10px] bg-green-900/30 text-green-400 px-2 py-1 rounded border border-green-900 uppercase font-bold tracking-wider">Station {seg.startStation || '0'} to {seg.endStation || '0'}</span>
                            {seg.photo && <img src={seg.photo} className="mt-2 h-16 w-16 object-cover rounded border border-green-900" />}
                        </div>
                        <div className="md:col-span-5">
                             <p className="text-sm text-zinc-300 font-bold font-mono mb-2">Dims: {seg.length}'(L) x {seg.width}'(W) x {seg.depth}"(D)</p>
                             <p className="text-sm text-zinc-500 font-mono font-bold"><span className="text-zinc-600">Formula:</span> {res.volumeCF.toFixed(1)}cf × {seg.density}pcf × {seg.percent/100} = <span className="text-white print:text-white font-black text-lg">{Math.round(res.totalLbs).toLocaleString()} lbs</span></p>
                        </div>
                        <div className="md:col-span-2 text-right">
                             <p className="font-black text-4xl text-green-500 leading-none tracking-tighter">{res.tons.toFixed(2)}</p>
                             <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mt-1">TONS</p>
                             <p className="text-[10px] text-zinc-400 mt-1">Rate: <span className="text-white print:text-white font-bold">{res.spreadRate.toFixed(1)}</span> lb/sy</p>
                        </div>
                        <div className="md:col-span-1 text-right print:hidden">
                            <button onClick={() => removeSegment(seg.id)} className="p-2 text-zinc-600 hover:text-red-500 transition-colors"><Icons.Trash2 size={20}/></button>
                        </div>
                    </div>
                )
            })}
            <div className="p-8 bg-zinc-900 border-t-2 border-green-600 flex justify-between items-center print:bg-black print:border-green-600">
                <div>
                    <span className="block text-green-500 font-black uppercase text-sm tracking-widest">Job Total Requirement</span>
                    <span className="text-xs text-zinc-500">Generated by Gilchrist QC App</span>
                </div>
                <span className="text-5xl font-black text-white print:text-white tracking-tighter">{totalTons.toFixed(2)} <span className="text-2xl text-zinc-500">TONS</span></span>
            </div>
         </div>
      )}
    </div>
    ...
    return (
        <div className="space-y-6 animate-in slide-in-from-bottom-4">
            <div className="flex justify-between items-center border-b border-zinc-800 pb-4 print:hidden">
                <h2 className="text-3xl font-black text-white uppercase tracking-tighter">Asphalt Yield</h2>
                <div className="text-xs bg-zinc-900 border border-zinc-800 px-3 py-1 rounded text-zinc-500 font-mono">Factor: 110 lb/sy-in</div>
            </div>
            <div className="print:hidden">
            ...
            <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-xl shadow-lg print:hidden">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-4">
                         <div className="grid grid-cols-2 gap-4 bg-black p-4 rounded-lg border border-zinc-800">
                            <InputField label="START STATION" placeholder="100+00" value={current.startStation || ''} onChange={(v) => setCurrent({...current, startStation: v})} />
                            <InputField label="END STATION" placeholder="110+00" value={current.endStation || ''} onChange={(v) => setCurrent({...current, endStation: v})} />
                        </div>
                    ...
                    <div>
                        ...
                        <div className="mt-4 p-8 bg-black rounded-xl text-center border border-zinc-800 h-64 flex flex-col justify-center">
                             <p className="text-xs font-bold text-zinc-500 uppercase mb-2 tracking-widest">Estimated Tonnage</p>
                             <p className="text-5xl font-black text-white">{calculate(current).tons.toFixed(2)}</p>
                             <p className="text-sm text-zinc-500 mt-4 font-bold">Trucks ({current.truckCapacity}t): <span className="text-white">{Math.ceil(calculate(current).tons / current.truckCapacity)}</span></p>
                        </div>
                    </div>
                </div>
            ...
            {segments.length > 0 && (
                <div className="bg-black border-2 border-green-600 rounded-xl overflow-hidden shadow-2xl print:border-green-600 print:shadow-none print:w-full print:absolute print:top-0 print:left-0 print:z-50 print:bg-black">
                   <style>{`@media print { body { -webkit-print-color-adjust: exact; print-color-adjust: exact; background-color: white !important; } .print\\:bg-black { background-color: #000 !important; } .print\\:text-white { color: #fff !important; } }`}</style>
                   <div className="p-6 border-b-2 border-green-600 bg-black print:bg-black flex justify-between items-center">
                        <div className="flex items-center gap-4">
                            <img src="/Gilchrist_Logo.png" alt="GILCHRIST" className="h-12 object-contain" onError={(e) => e.target.style.display='none'}/>
                            <div>
                                <h3 className="font-black text-white print:text-white text-xl uppercase tracking-tighter leading-none">Paving Schedule</h3>
                                <p className="text-green-500 text-xs font-bold uppercase tracking-widest mt-1">Asphalt Yield Report</p>
                            </div>
                        </div>
                       <div className="print:hidden">
                           <button onClick={() => window.print()} className="flex items-center gap-2 text-[10px] font-bold uppercase text-black bg-white px-4 py-2 rounded hover:bg-gray-200 transition-colors"><Icons.Printer size={14}/> Print Report</button>
                   ...
                   {segments.map(seg => {
                       ...
                       return (
                           <div key={seg.id} className="p-6 border-b border-zinc-800 grid grid-cols-1 md:grid-cols-12 gap-6 items-center bg-black print:bg-black hover:bg-zinc-900/30 transition-colors">
                               <div className="md:col-span-4">
                                   <span className="block font-black text-2xl text-white print:text-white uppercase mb-1">HMA Paving</span>
                                   <span className="inline-block text-[10px] bg-green-900/30 text-green-400 px-2 py-1 rounded border border-green-900 uppercase font-bold tracking-wider">Station {seg.startStation || '0'} to {seg.endStation || '0'}</span>
                                   {seg.photo && <img src={seg.photo} className="mt-2 h-16 w-16 object-cover rounded border border-green-900" />}
                               </div>
                               <div className="md:col-span-5">
                                    <p className="text-sm text-zinc-300 font-bold font-mono mb-2">Dims: {seg.length}'(L) x {seg.width}'(W) x {seg.thickness}"(T)</p>
                                    <p className="text-sm text-zinc-500 font-mono font-bold"><span className="text-zinc-600">Formula:</span> {res.sy.toFixed(1)}sy × {seg.thickness}" × {seg.density} = <span className="text-white print:text-white font-black text-lg">{Math.round(res.totalLbs).toLocaleString()} lbs</span></p>
                               </div>
                               <div className="md:col-span-2 text-right">
                                    <p className="font-black text-4xl text-green-500 leading-none tracking-tighter">{res.tons.toFixed(2)}</p>
                                    <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mt-1">TONS</p>
                   ...
                   <div className="p-8 bg-zinc-900 border-t-2 border-green-600 flex justify-between items-center print:bg-black print:border-green-600">
                       <div>
                            <span className="block text-green-500 font-black uppercase text-sm tracking-widest">Total Tonnage</span>
                            <span className="text-xs text-zinc-500">Generated by Gilchrist QC App</span>
                       </div>
                       <span className="text-5xl font-black text-white print:text-white tracking-tighter">{totalTons.toFixed(2)} <span className="text-2xl text-zinc-500">TONS</span></span>
                   </div>
...
function InspectionsView({ data, onDelete, onGenerateReport, onOpenModal }) {
  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-bottom-4">
      ...
      <div className="flex justify-between items-center border-b border-zinc-800 pb-4 print:hidden">
        <h2 className="text-2xl font-black text-white uppercase tracking-tighter">Quality Inspections</h2>
        <button onClick={onOpenModal} className="bg-green-600 text-white px-4 py-2 rounded font-bold text-xs hover:bg-green-500 flex items-center gap-2 uppercase tracking-wide transition-colors">
          <Icons.Plus size={16} /> New Inspection
        </button>
      </div>
        ...
        {filteredData.map((insp) => (
          <div key={insp.id} className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden p-6 hover:border-zinc-700 transition-colors">
            <div className="flex justify-between items-start mb-2">
              <div><h3 className="text-lg font-bold text-white">{insp.title}</h3><p className="text-xs text-zinc-500 font-bold mt-1 uppercase">{insp.location} • {insp.date}</p></div>
            ...
            <div className="flex gap-3 pt-4 border-t border-zinc-800">
               <button onClick={() => onDelete(insp.id)} className="text-red-500 text-xs font-bold uppercase flex items-center gap-2 hover:text-red-400"><Icons.Trash2 size={14} /> Delete</button>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="hidden print:block bg-black border-2 border-green-600 rounded-xl overflow-hidden print:w-full print:absolute print:top-0 print:left-0 print:z-50">
          <div className="p-6 border-b-2 border-green-600 bg-black print:bg-black flex justify-between items-center">
                <div className="flex items-center gap-4">
                    <img src="/Gilchrist_Logo.png" alt="GILCHRIST" className="h-12 object-contain" onError={(e) => e.target.style.display='none'}/>
                    <div>
                        <h3 className="font-black text-white print:text-white text-xl uppercase tracking-tighter leading-none">Inspection Report</h3>
                        <p className="text-green-500 text-xs font-bold uppercase tracking-widest mt-1">Quality Control Log</p>
                    </div>
                </div>
          </div>
          {filteredData.map((insp) => (
             <div key={insp.id} className="p-6 border-b border-zinc-800 bg-black print:bg-black">
                <div className="flex justify-between items-start mb-2">
                    <div>
                        <h3 className="text-xl font-black text-white uppercase">{insp.title}</h3>
                        <p className="text-sm text-zinc-400 font-bold mt-1 uppercase">{insp.location} | {insp.date}</p>
                    </div>
                    <div className="text-right">
                        <span className={`px-3 py-1 rounded text-sm font-bold border uppercase ${insp.result === 'Pass' ? 'text-green-500 border-green-600' : 'text-red-500 border-red-600'}`}>{insp.result}</span>
                    </div>
                </div>
                <div className="mt-4 p-4 border border-zinc-800 rounded bg-zinc-900/50">
                    <p className="text-sm text-white italic">"{insp.notes}"</p>
                </div>
                {insp.photo && (
                    <div className="mt-4">
                        <img src={insp.photo} className="h-32 object-contain rounded border border-green-900" />
                        <p className="text-[10px] text-zinc-500 uppercase mt-1">Attached Evidence</p>
                    </div>
                )}
             </div>
          ))}
          <div className="p-8 bg-zinc-900 border-t-2 border-green-600 flex justify-between items-center print:bg-black print:border-green-600">
             <span className="text-xs text-zinc-500">Generated by Gilchrist QC App</span>
             <span className="text-white font-bold uppercase">Total Items: {filteredData.length}</span>
          </div>
      </div>
    </div>
  );
...
function MaterialTestsView({ data, onDelete, onOpenModal }) {
  return (
    <div className="space-y-6 animate-in slide-in-from-bottom-4">
      ...
      <div className="flex justify-between items-center border-b border-zinc-800 pb-4 print:hidden">
        <h2 className="text-2xl font-black text-white uppercase tracking-tighter">Material Tests</h2>
        <button onClick={onOpenModal} className="bg-green-600 text-white px-4 py-2 rounded font-bold text-xs hover:bg-green-500 flex items-center gap-2 uppercase tracking-wide transition-colors">
          <Icons.Plus size={16} /> New Test
        </button>
      </div>
          ...
          <div key={test.id} className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden p-6 hover:border-zinc-700 transition-colors">
            <div className="flex justify-between items-start mb-2">
              <div>
                  ...
                  <p className="text-xs text-zinc-500 font-bold mt-1 uppercase">{test.procedure} • {test.date}</p>
              </div>
              <Badge status={test.result} />
            </div>
            <div className="flex justify-between items-center bg-black p-3 rounded border border-zinc-800 mb-4">
                <span className="text-xs font-bold text-zinc-500 uppercase">Result Value</span>
                <span className="text-xl font-black text-white">{test.value} <span className="text-sm text-green-500">{test.unit}</span></span>
            </div>
            <div className="flex gap-3 pt-4 border-t border-zinc-800">
               <button onClick={() => onDelete(test.id)} className="text-red-500 text-xs font-bold uppercase flex items-center gap-2 hover:text-red-400"><Icons.Trash2 size={14} /> Delete</button>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="hidden print:block bg-black border-2 border-green-600 rounded-xl overflow-hidden print:w-full print:absolute print:top-0 print:left-0 print:z-50">
          <div className="p-6 border-b-2 border-green-600 bg-black print:bg-black flex justify-between items-center">
                <div className="flex items-center gap-4">
                    <img src="/Gilchrist_Logo.png" alt="GILCHRIST" className="h-12 object-contain" onError={(e) => e.target.style.display='none'}/>
                    <div>
                        <h3 className="font-black text-white print:text-white text-xl uppercase tracking-tighter leading-none">Material Test Report</h3>
                        <p className="text-green-500 text-xs font-bold uppercase tracking-widest mt-1">Laboratory & Field Data</p>
                    </div>
                </div>
          </div>
          {data.map((test) => (
             <div key={test.id} className="p-6 border-b border-zinc-800 bg-black print:bg-black grid grid-cols-2 items-center">
                <div>
                    <h3 className="text-xl font-black text-white uppercase">{test.title}</h3>
                    <p className="text-sm text-zinc-400 font-bold mt-1 uppercase">{test.procedure} | {test.location}</p>
                </div>
                <div className="text-right">
                    <span className="block text-4xl font-black text-white">{test.value} <span className="text-lg text-green-500">{test.unit}</span></span>
                    <span className={`inline-block mt-2 px-3 py-1 rounded text-xs font-bold border uppercase ${test.result === 'Pass' ? 'text-green-500 border-green-600' : 'text-red-500 border-red-600'}`}>{test.result}</span>
                </div>
             </div>
          ))}
          <div className="p-8 bg-zinc-900 border-t-2 border-green-600 flex justify-between items-center print:bg-black print:border-green-600">
             <span className="text-xs text-zinc-500">Generated by Gilchrist QC App</span>
             <span className="text-white font-bold uppercase">Total Tests: {data.length}</span>
          </div>
      </div>
    </div>
  );
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {categories.map((cat, idx) => (
          <a key={idx} href={cat.url} target="_blank" rel="noreferrer" className="bg-zinc-900 p-5 rounded-xl border border-zinc-800 flex items-center gap-5 hover:border-green-600 transition-colors cursor-pointer group">
            <div className="p-3 bg-black rounded-lg group-hover:bg-green-900/20 transition-colors">{cat.icon}</div>
            <div><h3 className="font-bold text-white text-base">{cat.title}</h3><p className="text-sm text-zinc-500 font-bold">{cat.desc}</p></div>
            <Icons.ExternalLink className="ml-auto text-zinc-600 group-hover:text-white" size={16}/>
          </a>
        ))}
      </div>
    </div>
...
export default function App() {
    ...
    switch (activeTab) {
      ...
      case 'soilcalc': 
        return <SoilCalcView 
          ...
          addSegment={() => {
            if (!currentSoil.length || !currentSoil.width) return;
            setSoilSegments([...soilSegments, {...currentSoil, id: Date.now()}]);
            setCurrentSoil({ ...currentSoil, startStation: '', endStation: '', length: '', width: '', depth: '', id: Date.now(), photo: null });
          }} 
          removeSegment={(id) => setSoilSegments(soilSegments.filter(s=>s.id!==id))} 
          current={currentSoil} 
          setCurrent={setCurrentSoil} 
        />;
      case 'asphaltcalc': 
        return <AsphaltCalcView 
          segments={asphaltSegments} 
          addSegment={() => {
             if (!currentAsphalt.length || !currentAsphalt.width) return;
             setAsphaltSegments([...asphaltSegments, {...currentAsphalt, id: Date.now()}]);
             setCurrentAsphalt({ ...currentAsphalt, startStation: '', endStation: '', length: '', width: '', thickness: '', id: Date.now(), photo: null });
          }} 
          removeSegment={(id) => setAsphaltSegments(asphaltSegments.filter(s=>s.id!==id))} 
          current={currentAsphalt} 
          setCurrent={setCurrentAsphalt} 
        />;
  ...
  return (
    <div className="min-h-screen bg-black font-sans text-zinc-300 pb-20 md:pb-0">
      ...
      <header className="bg-black border-b border-zinc-800 px-6 py-4 flex items-center justify-between sticky top-0 z-50 shadow-2xl print:hidden">
        <div className="flex items-center gap-4">
          <img src="/Gilchrist_Logo.png" alt="GILCHRIST CONSTRUCTION COMPANY" className="h-12 object-contain" />
        </div>
        <div className="flex items-center gap-2 bg-zinc-900 px-3 py-1.5 rounded border border-zinc-800"><div className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></div><span className="text-xs font-bold text-zinc-400 uppercase">Logged In: Bernie</span></div>
      </header>
      ...
      <div className="bg-black border-b border-zinc-800 px-4 py-2 overflow-x-auto print:hidden">
        <nav className="flex gap-2 min-w-max">
          {[
            ...
            { id: 'soilcalc', label: 'Soil Calc', icon: <Icons.Calculator size={16} /> },
            { id: 'asphaltcalc', label: 'Asphalt Yield', icon: <Icons.Truck size={16} /> },
            { id: 'inspections', label: 'Inspections', icon: <Icons.ClipboardCheck size={16} /> },
            { id: 'tests', label: 'Material Testing', icon: <Icons.FlaskConical size={16} /> },
            { id: 'forms', label: 'LADOTD Forms', icon: <Icons.BookOpen size={16} /> },
            { id: 'ai', label: 'AI Assistant', icon: <Icons.Bot size={16} /> },
          ].map((item) => (
            <button key={item.id} onClick={() => setActiveTab(item.id)} className={`flex items-center gap-2 px-4 py-3 rounded text-xs font-black uppercase tracking-wide transition-all ${activeTab === item.id ? 'bg-green-600 text-white shadow-lg shadow-green-900/20 scale-105' : 'bg-zinc-900 text-zinc-500 hover:text-white hover:bg-zinc-800'}`}>{item.icon}{item.label}</button>
          ))}
        </nav>
      </div>
